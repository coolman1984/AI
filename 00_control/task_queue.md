...
| IS2 | harden Excel/CSV ingestion into reusable spine | Builder→Evaluator | In Progress | High | sample + real exports, IS1 contract | reusable tracked CSV path + upcoming multi-sheet typed ingestion + reject flow | test-first + pytest `tests/test_data_pipeline.py` | E18 |
...
**Next best action:** two-track move — externally, receive one real export for **P2.2**; internally, continue **IS2** by pushing the existing CSV path from tracked ingest into typed reusable ingestion instead of leaving it half-modernized.
