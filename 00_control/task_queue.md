...
**Next best action:** two-track move — externally, receive one real export for **P2.2**; internally, continue **IS2** with **IS2.3** (multi-sheet Excel) or **IS2.4** (per-column type inference) — the reject flow (IS2.2) is now live on the CSV path.
